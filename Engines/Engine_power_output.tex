...
% choose the document margins
\usepackage[
top    = 2.00cm,
bottom = 2.00cm,
left   = 2.00cm,
right  = 2.00cm,
headsep= 0.15cm]{geometry}
\usepackage{lscape}
\usepackage{graphicx}
% \graphicspath{ {images/} }
...
\begin{document}
	...
	To make it simple, we consider that in each cylinder the pressure profile during combustion stroke is the same. 
	\begin{itemize}
		\item Step 1 : compute power produced by 1 piston during the combustion stroke, when the piston head is moving down
	\begin{equation}
		\alpha_{c} \in [90 \degree, 270 \degree] \text{and} \in [450 \degree, 630 \degree]
	\end{equation}
		\item Step 2 : shift the power profile for the piston according to its number and firing order.
	\end{itemize}
	For example, the firing order of an I4 is 1-3-2-4. The combustion stroke starts at this angle :  
	\begin{eqnarray}
		alpha_1 = alpha_{cc} = 90\degree\\
		alpha_3 = alpha_{cc} + 180\degree\\
		alpha_2 = alpha_{cc} + 360\degree\\
		alpha_4 = alpha_{cc} + 540\degree
	\end{eqnarray}
	...
	The displacement of a cylinder is the difference between max and min volume of the combustion chamber : \\
	$V_{\min}$ is the volume when the piston is at TDC\\
	$V_{max}$ is the volume when the piston is at BDC\\
	\begin{eqnarray}
		disp_{cylinder} = V_{max} - V_{min} = stroke \cdot S_{piston} = stroke \cdot \pi (\frac{D}{2})^2 
	\end{eqnarray}
	
	\begin{equation}
		disp_{engine} = disp_{cylinder} \cdot nb_{pistons} = stroke \cdot \pi (\frac{D}{2})^2 \cdot nb_{pistons}
	\end{equation}
	
	
	\subsection{Piston and crankshaft movement}
	The following figure shows the engine most important moving parts in action : the piston head, connecting rod and the crankshaft. \\
	...
	This movement is initiated by the power stroke, where the pressure, built in the combustion chamber, is converted to force that pushes the piston head downward. At the end of the connecting rod, this force generates a torque that will rotate the crankshaft, in the trigonometric direction in our case.\\
	
	The main referential is orthogonal and centered on the crankshaft rotation axis: 
	\begin{itemize}
		\item $X$ to the right
		\item $Y$ to the top
		\item $Z$ perpendicular to the plan formed by x and y .
		\item Angles are counted starting from $X$-axis, and are positive in the trigonometric direction
	\end{itemize}
		
	Let's define the angle and position vector of each component: 
	\begin{enumerate}
		\item Crankshaft crankin journal position: 
	\begin{eqnarray}
		\overline{CJ} = L_{CJ}\\
		\vec{CJ} = L_{CJ} \cdot 
		\begin{bmatrix}
		...
		\alpha_{c} \in [0, 2 \pi] 
	\end{eqnarray} 
		\item Position of the bottom part of the connecting rod : 
	\begin{eqnarray}
		\overline{PJ} = L_{PJ}\\
		\vec{PJ} = L_{PJ} \cdot  
			\begin{bmatrix}
			...
			0
		\end{bmatrix}_{R_{1}} \\
		\theta_{cr} \in [\theta_{cr}^{min}, \theta_{cr}^{max}]
	\end{eqnarray}
		\item Position of the piston head pivot : 
	\begin{eqnarray}
	\vec{CP} = L_{CP} \cdot  
	\begin{bmatrix}
		0 \\
		...
		0
	\end{bmatrix}_{R_{1}} \\
	L_{CP} \text{ $\in$ } [BDC, TDC] = [BDC, BDC + stroke]
	\end{eqnarray}
	\end{enumerate}
	\subsection{Newton-Raphson}
	The Newton-Raphson method is used to find the angle of the connecting rod. \\
	Starting from the center of the crankshaft, the previous vectors are linked together to form a vector loop. This loop must be closed, meaning the sum of the vectors must be equal to 0.\\	
	
	The following system of two equations must be solved
	\begin{eqnarray}
	...
	If we analyze the equation, we notice that three variables are not defined. To solve this problem, we set one of them as piloted or driving variable. This means that during the resolution of the system, the value of this variable is fixed. In our case, this driving variable is the crankshaft angle $\alpha_c$, which value varies from 0 to 360$\degree$.\\
	
	The two unknown variables are : 
	\begin{itemize}
		\item angle of the connecting rod : $\theta_{cr}$
		\item position of the piston head : $L_{CP}$
	\end{itemize}
	The iterations of the Newton-Raphson method are executed until the system is solved, meaning finding the values of the unknown variables until the equation system is equal to $\vec{0}$. The result will be the value of the unknown variables as function of the crankshaft angle. \\
		
	With this we can define the range $[\theta_{cr}^{min}, \theta_{cr}^{max}]$. 
	
	After the system is solved, the value of each variable must be within the specified range. 
	...
	the thrust force in the connecting rod referential is computed by rotating the previous vector with a rotation matrix . The angle is the connecting rod angle $\theta_{cr}$, found using the results of the previous section, 
	\[
	\vec{F}_{thrust}^{R_{c c}}=F_{thrust} \cdot\left[\begin{array}{c}
		\cos \left(\theta_{cr}\right)  \tag{5}\\
		\sin \left(\theta_{cr}\right) \\
		0
	\end{array}\right]_{R_{cr}}
	\]
	
	$\theta_{cr}$ : connecting rod angle\\
