...
\fancyhead[L]{\slshape Ballistics}
...
\section{Ballistic trajectory}
\subsection{Perfect parabolic trajectory}
first exercise, draw a perfect parabolic trajectory of a projectile.
example : throwing a rock with hand

initial situation : the moment the rock leaves the hand 
in that situation : the rock has a velocity $\vec{v}$ with an angle of $angle$ from the ground. 
we only consider the gravity as force applied on the rock during the movement until it reaches the ground. 
...
\begin{eqnarray}
	x = \frac{1}{2} \cdot a_x \cdot t^2 + v_x \cdot t + x_0 \\
	y = \frac{1}{2} \cdot a_y \cdot t^2 + v_y \cdot t + y_0 \\
	z = \frac{1}{2} \cdot a_z \cdot t^2 + v_z \cdot t + z_0 \\
\end{eqnarray}
...
\subsection{Realistic trajectory}

Phase 1 : charge ignition phase : from the moment the charge is set off until the energy released by the charge starts pushing the projectile. In most case, this lasts few milliseconds.
if the projectile is launched from a canon (artillery, tanks, infantry weapons), compute the energy released by the charge and compute the force that will push the projectile
if it is a simple throwing from the hand, just skip this part and consider the human force that will throw the projectile

Phase 2 : propulsion phase : from the time the projectile moves from a stationary position to moving state. this used for launch from canon. 

convert charge energy to pressure and study the propulsion force as the projectile is moving inside the cannon. 
(tip : as the projectile moves, the propulsion pressure will decrease as the volume is lower and the ignition energy is diminishing)
: study the force that will make the projectile move, -> compute the state of the projectile when it starts moving :
Example : projectile state when it exits the canon barrel | stone state when it leaves the hand of the thrower

Artillery : convert the propulsion charge energy to a propulsion force

Phase 2 : compute the trajectory

use cinematic relation that was mentioned before. 
at each timestep $t_i$, compute the forces applied on the projectile -> $\vec{a} = \frac{ \sum F}{m}$

compute the difference of position due to the increase/decrease of the acceleration

\begin{eqnarray}
	ts = t_{ii+1} - t_i\\
	\Delta \vec{r} = \int_{t_i}^{t_{i+1}}  \vec{v}(t) \cdot dt = \int_{t_i}^{t_{i+1}}  (\vec{a}(t) \cdot t + \vec{v_0}) dt\\
	\Delta \vec{r}_i = (\frac{1}{2} \cdot \vec{a}(t) \cdot t^2 + \vec{v_0} \cdot t + \vec{r_0}) \bigg|_{t_i}^{t_{i+1}}\\
	\vec{r}(t_{i+1}) = \vec{r}(t_{i+1}) + \Delta \vec{r}_i
\end{eqnarray}

The timestep $ts$ should be as low as possible, depending on the system computation speed and the data speed of the sensor. 
