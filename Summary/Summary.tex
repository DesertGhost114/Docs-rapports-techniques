...
\url{https://fr.wikipedia.org/wiki/Quantit%C3%A9_de_mouvement}
	...
	\end{equation}
	Earth gravitational acceleration depending on altitude $h$: 
	\begin{equation}
	g=G\cdot \frac{M_{Earth}}{(Radius_{Earth}+h)^2}[\frac{m}{s^2}]
	\end{equation}
	...
	Euler force
	\begin{equation}
		F_E = - m \frac{d\vec{\omega}}{dr} x \vec{r}
	\end{equation}
	
	Drag force
	\begin{eqnarray}
		F_d = \frac{- C_x \cdot \rho \cdot S \cdot v^2}{2}\\
		\vec{F_d} = \frac{- C_x \cdot \rho \cdot S}{2} \cdot \begin{bmatrix}
			v_{x}^2\\
			v_{y}^2 \\
			v_{z}^2
		\end{bmatrix}\cdot \frac{\vec{v}}{||v||} 
	\end{eqnarray}
	Wind force
	\begin{equation}
		\vec{F_w} = \frac{C_x \cdot \rho \cdot S}{2} \cdot \begin{bmatrix}
			v_{wx}^2\\
			v_{wy}^2 \\
			v_{wz}^2
		\end{bmatrix}\cdot \frac{\vec{v_w}}{||v_w||} 
	\end{equation}
	
	For the vector expressions, the $C_x$ is also expressed as a vector, as its value is different depending on the axis
	\begin{equation}
		C_x -> \vec{C} = \begin{bmatrix}
			C_x\\
			C_y \\
			C_z
		\end{bmatrix}
	...
	Corriolis force
	\begin{equation}
		F_c = - 2 \cdot m \cdot \vec{\omega} x \vec{v} 
	\end{equation}
	\subsection{Pressure}
